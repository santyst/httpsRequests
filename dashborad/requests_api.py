...
def https_get_rq (ip_addr, passwd, parameters):
    try:
        #se realiza la petición get request con los parámetros y la autenticación de usuario y un timeout de 1 segundo
        response = requests.get('https://'+ip_addr+'/vb.htm',
                    verify=False,
                    params=parameters,
                    auth=('admin',passwd),
                    timeout=1
        ...
        #si no fue posible hacer el get request se imprime fail
        print(f'{ip_addr} ...Fail...')
        return (-1)
...
def api_device_brightness():
    #Se obtine el json con los parametros desde el get request
    #datajs=request.get_json()
    #print(datajs)
    payload = {'getscreenluminancevalue': '',#intensidad de la luz
			'getledenable': '',
			}
    i = '190.156.215.178'
    password = '12345'
    #i = '10.129.123.236'
    #password = 'f33gg7er'
    respuesta_get = https_get_rq('190.156.215.178','12345',payload)
    print(respuesta_get)
    if respuesta_get == -1:
        error = {"error": "la conexión falló"}
        return jsonify(error)
    
    elif respuesta_get.find('401 Unauthorized') != -1:
        error = {"error": "fallaron las credenciales de autenticación"}
        return jsonify(error)

    elif respuesta_get.find('getscreenluminancevalue') < 0:
        error = {"error": "error de sintaxis en los conmandos"}
        return jsonify(error)
    elif respuesta_get.find('getledenable') < 0:
        error = {"error": "error de sintaxis en los conmandos"}
        return jsonify(error)
    else :
        #separación de la respuesta por saltos de línea
        respuesta_get = respuesta_get.split('\n')
        arreglo_temp = []
        #se recorren los elementos de la lista generada por split()
        for element in respuesta_get:
            try:
                #separación del elemento de la lista por caracter =
                #el primer elemento es el comando y el segundo el valor
                #OK getscreenluminancevalue=72
                value = element.split('=')[1]
                #se almacena el valor en un arreglo
                arreglo_temp.append(value)
            except:
                #si el elemento de la lista no tiene =, se ignora
                #esto siempre sucede con el último item de la lista que esta vacío ''
                pass
        #los valores se almacenan en arrglo_temp según el orden de la variable payload
        Brightness['getscreenluminancevalue'] = arreglo_temp[0]
        Brightness['getledenable'] = arreglo_temp[1]
        #Brightness[0]=arreglo_temp[0]
